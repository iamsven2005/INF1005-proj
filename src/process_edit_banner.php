...
if ($title === '') {
    header("Location: edit_banner.php?id=" . $bannerID);
    exit();
}

if ($startDate !== null && $endDate !== null && $startDate > $endDate) {
    header("Location: edit_banner.php?id=" . $bannerID . "&error=invalid_dates");
    exit();
}
...
$sql = "UPDATE PromotionalBanners
        SET title = ?, subtitle = ?, imagePath = ?, ctaText = ?, ctaUrl = ?, startDate = ?, endDate = ?, is_active = ?
        WHERE bannerID = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("sssssssii", $title, $subtitle, $imagePath, $ctaText, $ctaUrl, $startDate, $endDate, $isActive, $bannerID);
$stmt->execute();
$stmt->close();
$conn->close();

header("Location: manage_banners.php?msg=updated");
exit();
